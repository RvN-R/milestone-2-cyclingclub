...
This is the main page for a small cycle team based in Wandsworth South West London. It is designed to be responsive and accessible on a range of devices and have interactive elements to help meet the clients key goals. The website features an About Us section, Club Rides section and finally a Sign Up section with a form so visitors can express interest in joining the club. 
...
The Club Ride section has a simple layout in order to maximise attention from those viewing it. In its center is a large map of south west london. Just below the map are two call to action buttons labelled Saturday Ride and Lunch Time Ride. The colours of these buttons mirror that used in the clubs logo and pages navigation bar. The buttons colour pallette inverts once a mouse runs over it. Once clicked the map displays the relevant route. In addition a new section appears with key information on the ride. This includes meeting point, ending point and a total distance in KM of the whole route. 
